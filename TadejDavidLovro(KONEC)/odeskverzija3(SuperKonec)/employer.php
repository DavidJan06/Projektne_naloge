...
<div class="main">
    <br />
    <a href='projects.php'>Nazaj na prelged projektov</a>;
    ...
    <table class="center" border="1">
        ...
        while($row=mysqli_fetch_array($query))
        {
            ...
            $uporab = "SELECT * 
                FROM projects p WHERE p.id_u=".$row['id_u'];
            $result2 = mysqli_query($link, $uporab);
            ...
            if ((($user2['id_u']) != $id_u) && (($user1['active']) == 0) && (($status_p['status_p']) == 0)){
                echo '<a href="employer_choose.php?id='.$row['id_p'].'"> Izberi </a>';
    ...
    <table class="tabcenter" border="1">
        ...
        <?php
        $sql2="SELECT p.id_p, p.title, p.start_date, p.end_date, p.description, u.first_name, u.last_name
                FROM projects p
        ...
        while($row=mysqli_fetch_array($query2))
        {
            ...
            $projekt = "SELECT p.id_p
                FROM projects p
                INNER JOIN users u
                ON u.id_u=p.id_u
                WHERE p.id_u=".$id_u;
            $result4 = mysqli_query($link, $akti);
            $user4 = mysqli_fetch_array($result4);
            
            
            echo '<td>';
            
            if($user3['id_p'] == $user4['id_p'] ){
